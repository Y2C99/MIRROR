...
_modes = Literal["raw", "A-A", "A-C", "CA-AC"]
...
class Hybrid_str():
    ...
    def truncated_by_es_base_offset(self,
        ...
        if mode == "raw":
            ...
            new_s1 = self.ecs_unpair[left_idx:es_idx] + "A" + self.ecs_unpair[
                es_idx + 1:right_idx + 1]
            new_s2 = self.ecs_pair[left_idx:es_idx] + " " + self.ecs_pair[
                es_idx + 1:right_idx + 1]
            new_s3 = self.es_pair[left_idx:es_idx] + " " + self.es_pair[
                es_idx + 1:right_idx + 1]
            new_s4 = self.es_unpair[left_idx:es_idx] + "*" + self.es_unpair[
                es_idx + 1:right_idx + 1]
        elif mode == "A-C":
            new_s1 = self.ecs_unpair[left_idx:es_idx] + "C" + self.ecs_unpair[
                es_idx + 1:right_idx + 1]
            new_s2 = self.ecs_pair[left_idx:es_idx] + " " + self.ecs_pair[
                es_idx + 1:right_idx + 1]
            new_s3 = self.es_pair[left_idx:es_idx] + " " + self.es_pair[
            ...
            if three_offset <= 3:
                raise ValueError(
                    "Three offset value should be >= 3 for CA-AC mode!")
            else:
                new_s1 = self.ecs_unpair[left_idx:es_idx-2] + \
                    "  CA  " + self.ecs_unpair[es_idx+4:right_idx+1]
                new_s2 = self.ecs_pair[left_idx:es_idx-2] + \
                    "NN  NN" + self.ecs_pair[es_idx+4:right_idx+1]
                new_s3 = self.es_pair[left_idx:es_idx-2] + \
                        "NN  NN" + self.es_pair[es_idx+4:right_idx+1]
                new_s4 = self.es_unpair[left_idx:es_idx-2] + \
                    "  *C  " + self.es_unpair[es_idx+4:right_idx+1]
...
def worker(row):
    ...
    for mode in modes:
        ...
        if five_mimic + five_arm > loc or len(fa) - 1 < loc + three_arm + three_mimic or hs == None:
            ...
            is_mimic = True
            if hybrid_hs != expected_mimic_hs:
                is_mimic = False
...
if __name__ == "__main__":
            ...
            Steps:
                1. get truncated RNAhybrid double strand structures from substrates based on the input fm,tm parameters.         
                   example:
                            ...
                            3'  C               AC 5'
                2. simulating ASO based on the feature types of truncated structures, this is the expected structure.
                   In most cases, the expected structure is same with the truncated structure.
                   expected structure:
                            5'  C               CC 3'
                ...
                4. Run RNAhybrid based on the ASO and target sequence. If the structure is different to the truncated structure,
                  ...
                  3'UACUGGAACCGG CCCCCCG*UUCGUCA  CACCACGUCCU5'
                  3'            U               AC           5'
                  In most cases, it is same with the expected structure.
    ...
    group_optional.add_argument("-s", dest="step", metavar="step length", required=False, type=int,help="Editing site step length on the mimic region(5'>3'),If set this value,the script will output multiple csv files and merge all the files to the final file")
    ...
    # group_optional.add_argument("-r", dest="reduce", help="If add this parameter,this will probably introduce A-G mismatch in ASO for non-target TAG,CAG motifs in the target fasta.", action="store_true", default=False)
    ...
    if not options.step:
        ...
        step = options.step
        all_fm_values = [i for i in range(
            five_mimic, total_length - 1 - five_arm - three_arm, step)]
        all_tm_values = [total_length - 1 - i -
                         five_arm - three_arm for i in all_fm_values]
        all_names = []
        all_logs = []
        all_csvs = []
        file_types = []
        for fm, tm in zip(all_fm_values, all_tm_values):
            all_names.append(options.out + "_len{length}_fa{fa}_ta{ta}_fm{fm}_tm{tm}.csv".format(
                length=total_length, fa=five_arm, ta=three_arm, fm=fm, tm=tm))
            all_logs.append(options.out + "_len{length}_fa{fa}_ta{ta}_fm{fm}_tm{tm}.log".format(
                length=total_length, fa=five_arm, ta=three_arm, fm=fm, tm=tm))
            file_types.append("len{length}_fa{fa}_ta{ta}_fm{fm}_tm{tm}".format(
                length=total_length, fa=five_arm, ta=three_arm, fm=fm, tm=tm))

        for out_csv_name, out_csv_log, file_type, fm, tm in zip(all_names, all_logs, file_types, all_fm_values, all_tm_values):
            five_mimic = fm  # modified
            three_mimic = tm  # modified
            manager = mp.Manager()
            out = manager.list()
            sys.stderr = open(out_csv_log, "w")
            with Pool(p) as pool:
            ...
            df_out = pd.concat(out, ignore_index=True)
            df_out = df_out.sort_values(by="level",ascending=False)
            df_out["file_type"] = file_type
            # write out log
            write_details(df_out)
            # df_out = df_out.drop_duplicates(subset="ASO_seq",keep="first",ignore_index=True)
            df_out.to_csv(out_csv_name, index=False)
            all_csvs.append(df_out)
        final_csv = pd.concat(all_csvs, ignore_index=True)
        final_csv.to_csv(options.out + "_len%d_merged.csv" % total_length, index=0)
